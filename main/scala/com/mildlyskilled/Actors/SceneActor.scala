...
class Scene private(val objects: List[Shape], val lights: List[Light]) extends Actor with ActorLogging{
      ...
      for (x <- 0 until w) {
        ...
        for (dx <- 0 until ss) {
          for (dy <- 0 until ss) {
            ...
            val dir = Vector(
              (sinf * 2 * ((x + dx.toFloat / ss) / w - .5)).toFloat,
              (sinf * 2 * (h.toFloat / w) * (.5 - (h + dy.toFloat / ss) / h)).toFloat,
